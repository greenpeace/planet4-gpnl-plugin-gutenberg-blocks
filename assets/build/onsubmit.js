...
/***/ "./assets/src/blocks/Petition/js/onsubmit.js":
/*!***************************************************!*\
  !*** ./assets/src/blocks/Petition/js/onsubmit.js ***!
  \***************************************************/
...
$('.gpnl-petitionform').on('submit', function () {
  var petition_form_element = this; // Get the  parameter from the petition form and add the action and CSRF protection
  ...
  post_form_value.ad_campaign = window[form_config].ad_campaign; // Disable the form so people can't resubmit

  toggleDisable($(petition_form_element).find('*')); // Do a ajax call to the wp_admin admin_ajax.php,
  ...
  $.ajax({
    ...
    error: function error() {
      ...
      cardback.append('<a href=\'' + window.location.href + '\' class="btn btn-primary btn-block"' + '">Probeer opnieuw</a>');
      cardflip(petition_form_element);
    }
  });
...
function cardflip(el) {
  ...
  var card = $(el.parentNode.parentNode); // first hide the signing button

  $(element.find('.signBtn')).toggle();
  $(element.find('.policies')).toggle(); // then cardflip the position attribute on the front and back of the card to support different lengths front and back

  $(parent.nextElementSibling).css('position', flip_positionattribute(parent.nextElementSibling));
  $(parent).css('position', flip_positionattribute(parent)); // then cardflip the card

  card.toggleClass('flipped'); // after animation hide the front

  card.one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function () {
    $(parent).toggle();
...
function fireShareEvent(platform, form_id) {
  if (typeof dataLayer !== 'undefined') {
    var form_config = 'petition_form_object_' + form_id;
    dataLayer.push({
      'event': 'petitiebutton',
      'conv_campaign': window[form_config].analytics_campaign,
      'conv_action': window[form_config].ga_action,
...
/******/ });
//# sourceMappingURL=onsubmit.js.map